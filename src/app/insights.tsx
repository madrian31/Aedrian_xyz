...
function IconClock({
  ...
  return (
    <FontAwesome6 name="clock" size={size} color={color} iconStyle="regular" />
  );
}
...
function computeInsights(allNotes: Note[]): InsightData {
  ...
  if (bestDayOfWeek)
    smartInsights.push({
      iconType: "calendar",
      text: `You write most on ${bestDayOfWeek}s`,
    });
  ...
  if (longestEntry > 0)
    smartInsights.push({
      iconType: "pen",
      text: `Your longest entry was ${longestEntry} words`,
    });
  ...
  if (topTags.length > 0)
    smartInsights.push({
      iconType: "tag",
      text: `Your most used tag is #${topTags[0].tag}`,
    });
...
function SmartInsightCards({
  insights,
}: {
  insights: { iconType: string; text: string }[];
}) {
  ...
  const renderIcon = (iconType: string) => {
    ...
    switch (iconType) {
      case "calendar":
        return <IconCalendar color={ACCENT} size={size} />;
      case "clock":
        return <IconClock color={ACCENT} size={size} />;
      case "trendUp":
        return <IconTrendUp size={size} />;
      case "trendDown":
        return <IconTrendDown size={size} />;
      case "pen":
        return <IconPen color={ACCENT} size={size} />;
      case "chat":
    ...
    <View style={{ gap: 8, marginBottom: 12 }}>
      {insights.map((insight, i) => (
        ...
        >
          ...
          <Text
            style={{ color: "#d1d5db", fontSize: 14, lineHeight: 20, flex: 1 }}
          >
            {insight.text}
          </Text>
        </View>
        ...
        <View style={{ flexDirection: "row", gap: 10, marginBottom: 20 }}>
          {[
            ...(data.bestDayOfWeek
              ? [
                  {
                    iconEl: <IconCalendar color={ACCENT} size={20} />,
